...
class EndorFilter:
    ...
    def repository_version(
        ...
        context_type_exclude: EndorContextType = None,
    ):
...
class EndorCtl:
    ...
    def get_project(self, git_url):
        ...
        resource_description = f"{resource_kind} with name '{git_url}' in namespace '{self.namespace}'"
    ...
    def get_repository_version(self, filter=None, retry=True):
        ...
        resource_description = f"{resource_kind} with filter '{filter}' in namespace '{self.namespace}'"
        repository_version = self.get_resources(resource_kind, filter=filter, retry=retry, page_size=1)[0]
    ...
    def get_scan_result(self, filter=None, retry=True):
        ...
        while True:
            ...
            if status == 'STATUS_SUCCESS':
                ...
                logger.info(f'   Scan is running. ScanResult uuid {uuid} for refs {refs} started at {start_time}.')
                logger.info(
    ...
    def get_sbom_for_commit(self, git_url: str, commit_sha: str) -> dict:
        ...
        try:
            ...
            package_version_uuids = [package_version['uuid'] for package_version in package_versions]
            package_version_names = [package_version['meta']['name'] for package_version in package_versions]
    ...
    def get_sbom_for_branch(self, git_url: str, branch: str) -> dict:
        ...
        try:
            ...
            filter_str = endor_filter.repository_version(
                project_uuid, ref=branch, context_type_exclude=EndorContextType.CI_RUN
            )
            repository_version = self.get_repository_version(filter_str)
