...
if [[ "${TEST_WITH_CSFLE:-}" != "ON" ]]; then
  ...
  # Load secrets generated by the "csfle-setup" function (setup.sh -> secrets-export.sh).
  pushd "${DRIVERS_TOOLS:?}/.evergreen/csfle"
  {
    ...
    export MONGOCXX_TEST_AWS_SECRET_ACCESS_KEY="${FLE_AWS_SECRET:?}"
    export MONGOCXX_TEST_AWS_ACCESS_KEY_ID="${FLE_AWS_KEY:?}"
    export MONGOCXX_TEST_AZURE_TENANT_ID="${FLE_AZURE_TENANTID:?}"
    export MONGOCXX_TEST_AZURE_CLIENT_ID="${FLE_AZURE_CLIENTID:?}"
    export MONGOCXX_TEST_AZURE_CLIENT_SECRET="${FLE_AZURE_CLIENTSECRET:?}"
    export MONGOCXX_TEST_GCP_EMAIL="${FLE_GCP_EMAIL:?}"
    export MONGOCXX_TEST_GCP_PRIVATEKEY="${FLE_GCP_PRIVATEKEY:?}"
    export MONGOCXX_TEST_AZURE_ACCESS_TOKEN="${CSFLE_AZURE_ACCESS_TOKEN:?}"
    export MONGOCXX_TEST_GCP_ACCESS_TOKEN="${CSFLE_GCP_ACCESS_TOKEN:?}"
