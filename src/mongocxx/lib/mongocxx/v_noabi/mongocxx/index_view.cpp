...
void append_to(v_noabi::index_model const& index, scoped_bson& doc) {
...
void append_to(v_noabi::options::index_view const& opts, scoped_bson& doc) {
...
bsoncxx::v1::document::value create_impl(
    ...
    if (commit_quorum) {
        ...
        auto sd_owner = v1::events::server_description::internal::make(
            libmongoc::client_select_server(client, true /* for_writes */, nullptr /* read_prefs */, &error));
        auto const sd = v1::events::server_description::internal::as_mongoc(sd_owner);

        if (!sd) {
            v_noabi::throw_exception<v_noabi::write_exception>(error);
        }

        // "commitQuorum" requires MongoDB 4.4+.
        static constexpr std::int32_t min_wire_version_for_commit_quorum = 9;

        bson_t const* const hello = libmongoc::server_description_hello_response(sd);
        bson_iter_t iter = {};
        if (!bson_iter_init_find(&iter, hello, "maxWireVersion") ||
            bson_iter_int32(&iter) < min_wire_version_for_commit_quorum) {
            throw v_noabi::write_exception{
...
bsoncxx::v1::stdx::optional<std::string> create_one_impl(
    ...
    auto const reply_owner = [&] {
        ...
        append_to(index, doc);
        return create_impl(
            coll, client, scoped_bson{BCON_NEW("0", BCON_DOCUMENT(doc.bson()))}.bson(), opts, commit_quorum);
    }();
...
bsoncxx::v1::document::value create_many_impl(
    ...
    return create_impl(coll, client, arr.bson(), opts, commit_quorum);
}
...
bsoncxx::v_noabi::stdx::optional<std::string> index_view::create_one(
    ...
    return create_one_impl(
        v1::indexes::internal::get_collection(_indexes),
        v1::indexes::internal::get_client(_indexes),
        model,
        doc.bson(),
        options.commit_quorum());
}
...
bsoncxx::v_noabi::document::value index_view::create_many(
    ...
    return bsoncxx::v_noabi::from_v1(create_many_impl(
        v1::indexes::internal::get_collection(_indexes),
        v1::indexes::internal::get_client(_indexes),
        indexes,
        doc.bson(),
...
void index_view::drop_one(bsoncxx::v_noabi::stdx::string_view name, v_noabi::options::index_view const& options) {
    scoped_bson doc;

    append_to(options, doc);

    drop_one_impl(v1::indexes::internal::get_collection(_indexes), name, doc.bson());
}
...
void index_view::drop_one(v_noabi::index_model const& model, v_noabi::options::index_view const& options) {
    if (auto const name = model.options()["name"]) {
...
void index_view::drop_all(v_noabi::options::index_view const& options) {
    scoped_bson doc;

    append_to(options, doc);

    drop_all_impl(v1::indexes::internal::get_collection(_indexes), doc.bson());
}

void index_view::drop_all(v_noabi::client_session const& session, v_noabi::options::index_view const& options) {
    scoped_bson doc;
