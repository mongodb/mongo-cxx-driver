...
void validate_gridfs_file(
    ...
    REQUIRE(static_cast<std::size_t>(files_doc->view()["length"].get_int64().value) == expected_contents.size());
    REQUIRE(files_doc->view()["chunkSize"].get_int32().value == expected_chunk_size);
    REQUIRE(files_doc->view()["filename"].get_string().value == bsoncxx::stdx::string_view(expected_file_name));
    ...
    auto num_chunks_div = std::lldiv(static_cast<std::int64_t>(expected_contents.size()), expected_chunk_size);
...
void validate_gridfs_file(
    ...
    for (auto&& chunks_doc :
         db["fs.chunks"].find(make_document(kvp("files_id", id)), options::find{}.sort(make_document(kvp("n", 1))))) {
...
std::vector<std::uint8_t> manual_gridfs_initialize(
    ...
    db["fs.files"].insert_one(
        make_document(kvp("_id", id), kvp("length", length), kvp("chunkSize", bsoncxx::types::b_int32{chunk_size})));
...
void manual_gridfs_initialize(
    ...
    for (std::int32_t i = 0; i < num_chunks; ++i) {
        ...
        bsoncxx::types::b_binary data = {
            bsoncxx::binary_sub_type::k_binary, static_cast<std::uint32_t>(bytes.size()), bytes.data()};

        db["fs.chunks"].insert_one(make_document(kvp("files_id", id), kvp("n", i), kvp("data", data)));

        bytes_written += bytes.size();
    }

    db["fs.files"].insert_one(make_document(
        kvp("_id", id), kvp("length", bytes_written), kvp("chunkSize", bsoncxx::types::b_int32{chunk_size})));
}
    ...
    auto run_test = [&]() {
        ...
        REQUIRE_THROWS_AS(
            bucket.upload_from_stream_with_id(
                bsoncxx::types::bson_value::view{bsoncxx::types::b_int32{0}}, "filename", &iss, upload_options),
            logic_error);
    };
...
TEST_CASE("downloading throws error when files document is corrupt", "[gridfs::bucket]") {
    ...
    auto run_test = [&]() {
        ...
        auto open_download_stream = [&bucket]() {
            return bucket.open_download_stream(bsoncxx::types::bson_value::view{bsoncxx::types::b_int32{0}});
        };
    ...
    SECTION("too many chunks") {
        chunk_size = bsoncxx::types::bson_value::view{bsoncxx::types::b_int32{1}};
        length = bsoncxx::types::bson_value::view{
            bsoncxx::types::b_int64{static_cast<std::int64_t>(std::numeric_limits<std::int32_t>::max()) + 1}};
        run_test();
    }
...
TEST_CASE("downloading throws error when chunks document is corrupt", "[gridfs::bucket]") {
...

    {
        ...
        db["fs.files"].insert_one(make_document(kvp("_id", 0), kvp("length", 1), kvp("chunkSize", 1)));
    }
...
TEST_CASE("mongocxx::gridfs::uploader::abort works", "[gridfs::uploader]") {
    ...
    gridfs::uploader uploader = bucket.open_upload_stream_with_id(bsoncxx::types::bson_value::view{id}, "file");
...
TEST_CASE("mongocxx::gridfs::uploader::write with arbitrary sizes", "[gridfs::uploader]") {
    ...
    auto run_test = [&]() {
        ...
        while (static_cast<std::int64_t>(bytes_written) < file_length) {
            std::size_t actual_write_size = static_cast<std::size_t>(std::min(
                static_cast<std::int64_t>(write_size), file_length - static_cast<std::int64_t>(bytes_written)));
...
TEST_CASE("gridfs::bucket::open_upload_stream_with_id works", "[gridfs::bucket]") {
    ...
    auto uploader = bucket.open_upload_stream_with_id(
        id, "file", options::gridfs::upload{}.chunk_size_bytes(static_cast<std::int32_t>(chunk_size)));
...
TEST_CASE("gridfs::bucket::upload_from_stream works", "[gridfs::bucket]") {
    ...
    auto validate = [&]() {
        validate_gridfs_file(
            db, "fs", id, "file", std::vector<std::uint8_t>{numbers.begin(), numbers.end()}, chunk_size);
    };
...
TEST_CASE("gridfs upload large file", "[gridfs::bucket]") {
    char const* const enable_slow_tests = std::getenv("MONGOCXX_ENABLE_SLOW_TESTS");

    if (!enable_slow_tests || bsoncxx::stdx::string_view{enable_slow_tests}.empty()) {
        SKIP("MONGOCXX_ENABLE_SLOW_TESTS is not enabled");
    }

    if (test_util::is_replica_set() || test_util::is_sharded_cluster()) {
        SKIP("This test is too slow when run against a replica set or sharded cluster");
    }
    ...
    static constexpr std::int32_t chunk_size{7 * 1024 * 1024};
    static constexpr auto length = static_cast<std::int64_t>(std::numeric_limits<std::uint32_t>::max()) + 2;

    auto uploader = bucket.open_upload_stream("large_file", options::gridfs::upload{}.chunk_size_bytes(chunk_size));

    std::vector<std::uint8_t> bytes;

    for (std::int64_t chunk_number = 0; chunk_number * chunk_size < length; ++chunk_number) {
        auto const current_chunk_size =
            static_cast<std::size_t>(std::min<std::int64_t>(chunk_size, length - chunk_number * chunk_size));
        bytes.assign(current_chunk_size, static_cast<std::uint8_t>(chunk_number % 200));
    ...
    validate_gridfs_file(
        ...
        id,
        "large_file",
        [&](bsoncxx::types::b_binary const& data, std::size_t chunk_number) {
            CAPTURE(chunk_number);

            CHECK(data.sub_type == bsoncxx::binary_sub_type::k_binary);
            CHECK(
                static_cast<std::int64_t>(data.size) ==
                std::min<std::int64_t>(
                    chunk_size, length - static_cast<std::int64_t>(chunk_number) * std::int64_t{chunk_size}));

            expected_bytes.assign(data.size, static_cast<std::uint8_t>(chunk_number % 200));
            REQUIRE(std::vector<std::uint8_t>{data.bytes, data.bytes + data.size} == expected_bytes);
        },
        chunk_size,
        length);
}
...
TEST_CASE("gridfs download large file", "[gridfs::bucket]") {
    ...
    auto const num_chunks_div = std::lldiv(length, chunk_size);
    ...
    bsoncxx::types::bson_value::view id{bsoncxx::types::b_oid{bsoncxx::oid{}}};
    manual_gridfs_initialize(db, num_chunks, chunk_size, id, [&, num_chunks](std::int32_t chunk_num) {
        std::int32_t current_chunk_size = chunk_size;

        if (chunk_num == num_chunks - 1) {
            current_chunk_size = static_cast<std::int32_t>(length - chunk_num * std::int64_t{chunk_size});
        }

        std::vector<std::uint8_t> bytes;
        bytes.assign(static_cast<std::size_t>(current_chunk_size), static_cast<std::uint8_t>(chunk_num % 200));
    ...
    std::vector<std::uint8_t> bytes;
    bytes.resize(std::size_t{chunk_size});

    std::size_t total_bytes_read = 0u;
    std::size_t chunk_number = 0u;
    std::vector<std::uint8_t> expected_bytes;

    while (std::size_t current_bytes_read = downloader.read(bytes.data(), std::size_t{chunk_size})) {
        CAPTURE(chunk_number);

        expected_bytes.assign(current_bytes_read, static_cast<std::uint8_t>(chunk_number % 200));
        REQUIRE(std::vector<std::uint8_t>{bytes.data(), bytes.data() + current_bytes_read} == expected_bytes);

        total_bytes_read += current_bytes_read;
        ++chunk_number;
    }

    REQUIRE(total_bytes_read == std::size_t{length});
}
