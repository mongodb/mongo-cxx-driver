...
TEST_CASE("A default constructed collection is false-ish", "[mongocxx][v_noabi][collection]") {
    collection c;
    REQUIRE(!c);
}

TEST_CASE("read_concern", "[mongocxx][v_noabi][collection]") {
    MOCK_DATABASE;
    ...
    collection_aggregate->interpose(
        [&](mongoc_collection_t*,
            ...
            if (opts.read_preference()) {
                REQUIRE(
                    mongoc_read_prefs_get_mode(read_preference) == static_cast<int>(opts.read_preference()->mode()));
            } else {
                REQUIRE(
...
TEST_CASE("count_documents", "[mongocxx][v_noabi][collection]") {
    MOCK_DATABASE;
    MOCK_COLLECTION; // "mocked_collection.dummy_collection"
...
TEST_CASE("estimated_document_count", "[mongocxx][v_noabi][collection]") {
    MOCK_DATABASE;
    MOCK_COLLECTION; // "mocked_collection.dummy_collection"
    ...
    collection_estimated_document_count->interpose(
        [&](mongoc_collection_t*, bson_t const* opts, mongoc_read_prefs_t const*, bson_t* reply, bson_error_t* error) {
...
TEST_CASE("find", "[mongocxx][v_noabi][collection]") {
    MOCK_DATABASE;
    MOCK_COLLECTION; // "mocked_collection.dummy_collection"
...
TEST_CASE("writes", "[mongocxx][v_noabi][collection]") {
    MOCK_DATABASE;
    MOCK_COLLECTION; // "mocked_collection.dummy_collection"
    MOCK_BULK;
    ...
    auto const filter_doc = make_document(kvp("_id", "wow"), kvp("foo", "bar"));
    auto const modification_doc = make_document(kvp("cool", "wow"), kvp("foo", "bar"));
    ...
    SECTION("Insert One Bypassing Validation") {
        expected_order_setting = true;
        bulk_operation_insert_with_opts->interpose(
            [&](mongoc_bulk_operation_t*, bson_t const* doc, bson_t const*, bson_error_t*) {
                bulk_operation_op_called = true;
    ...
    SECTION("Update Many Error") {
        ...
        REQUIRE_THROWS_AS(mongo_coll.update_many(filter_doc.view(), modification_doc.view()), mongocxx::logic_error);
        REQUIRE(!bulk_operation_execute_called);
        perform_checks();
    }
    ...
    SECTION("Replace One Error") {
        ...
        REQUIRE_THROWS_AS(mongo_coll.replace_one(filter_doc.view(), modification_doc.view()), mongocxx::logic_error);
        REQUIRE(!bulk_operation_execute_called);
        perform_checks();
    }
