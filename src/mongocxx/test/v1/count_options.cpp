...
TEST_CASE("ownership", "[mongocxx][v1][count_options]") {
    auto const source_value = 123;
    auto const target_value = 456;

    count_options source;
    count_options target;
...
TEST_CASE("default", "[mongocxx][v1][count_options]") {
    count_options const opts;
...
TEST_CASE("collation", "[mongocxx][v1][count_options]") {
    ...
    CHECK(count_options{}.collation(v.value()).collation() == v.view());
}

TEST_CASE("hint", "[mongocxx][v1][count_options]") {
    auto const v = GENERATE(values({
        v1::hint{"abc"},
        v1::hint{scoped_bson{R"({"x": 1})"}.value()},
    }));

    CHECK(count_options{}.hint(v).hint() == v);
}

TEST_CASE("comment", "[mongocxx][v1][count_options]") {
    ...
    CHECK(count_options{}.comment(v).comment() == v);
}

TEST_CASE("limit", "[mongocxx][v1][count_options]") {
    auto const v = GENERATE(values({
    ...
    CHECK(count_options{}.limit(v).limit() == v);
}

TEST_CASE("max_time", "[mongocxx][v1][count_options]") {
    ...
    CHECK(count_options{}.max_time(v).max_time() == v);
}

TEST_CASE("skip", "[mongocxx][v1][count_options]") {
    auto const v = GENERATE(values({
        std::int64_t{INT64_MIN},
        std::int64_t{-1},
        std::int64_t{0},
    ...
    CHECK(count_options{}.skip(v).skip() == v);
}

TEST_CASE("read_preference", "[mongocxx][v1][count_options]") {
