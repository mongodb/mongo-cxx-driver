...
TEST_CASE("ownership", "[mongocxx][v1][delete_one_options]") {
    delete_one_options source;
    delete_one_options target;
...
TEST_CASE("default", "[mongocxx][v1][delete_one_options]") {
...
TEST_CASE("collation", "[mongocxx][v1][delete_one_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(delete_one_options{}.collation(v.value()).collation() == v.view());
}

TEST_CASE("read_concern", "[mongocxx][v1][delete_one_options]") {
    ...
    CHECK(delete_one_options{}.read_concern(v).read_concern() == v);
}

TEST_CASE("write_concern", "[mongocxx][v1][delete_one_options]") {
    ...
    CHECK(delete_one_options{}.write_concern(v).write_concern() == v);
}

TEST_CASE("hint", "[mongocxx][v1][delete_one_options]") {
    auto const v = GENERATE(values({
        v1::hint{"abc"},
        v1::hint{scoped_bson{R"({"x": 1})"}.value()},
    }));

    CHECK(delete_one_options{}.hint(v).hint() == v);
}

TEST_CASE("let", "[mongocxx][v1][delete_one_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(delete_one_options{}.let(v.value()).let() == v.view());
}

TEST_CASE("comment", "[mongocxx][v1][delete_one_options]") {
    ...
    CHECK(delete_one_options{}.comment(v).comment() == v);
}
