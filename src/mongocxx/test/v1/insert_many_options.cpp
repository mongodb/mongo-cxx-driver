...
TEST_CASE("ownership", "[mongocxx][v1][insert_many_options]") {
    insert_many_options source;
    insert_many_options target;
...
TEST_CASE("default", "[mongocxx][v1][insert_many_options]") {
...
TEST_CASE("bypass_document_validation", "[mongocxx][v1][insert_many_options]") {
...
TEST_CASE("write_concern", "[mongocxx][v1][insert_many_options]") {
    ...
    CHECK(insert_many_options{}.write_concern(v).write_concern() == v);
}

TEST_CASE("ordered", "[mongocxx][v1][insert_many_options]") {
    auto const v = GENERATE(false, true);

    CHECK(insert_many_options{}.ordered(v).ordered() == v);
}

TEST_CASE("comment", "[mongocxx][v1][insert_many_options]") {
    ...
    CHECK(insert_many_options{}.comment(v).comment() == v);
}
