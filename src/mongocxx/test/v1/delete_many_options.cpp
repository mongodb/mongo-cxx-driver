...
TEST_CASE("ownership", "[mongocxx][v1][delete_many_options]") {
    delete_many_options source;
    delete_many_options target;
...
TEST_CASE("default", "[mongocxx][v1][delete_many_options]") {
...
TEST_CASE("collation", "[mongocxx][v1][delete_many_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(delete_many_options{}.collation(v.value()).collation() == v.view());
}

TEST_CASE("read_concern", "[mongocxx][v1][delete_many_options]") {
    ...
    CHECK(delete_many_options{}.read_concern(v).read_concern() == v);
}

TEST_CASE("write_concern", "[mongocxx][v1][delete_many_options]") {
    ...
    CHECK(delete_many_options{}.write_concern(v).write_concern() == v);
}

TEST_CASE("hint", "[mongocxx][v1][delete_many_options]") {
    auto const v = GENERATE(values({
        v1::hint{"abc"},
        v1::hint{scoped_bson{R"({"x": 1})"}.value()},
    }));

    CHECK(delete_many_options{}.hint(v).hint() == v);
}

TEST_CASE("let", "[mongocxx][v1][delete_many_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(delete_many_options{}.let(v.value()).let() == v.view());
}

TEST_CASE("comment", "[mongocxx][v1][delete_many_options]") {
    ...
    CHECK(delete_many_options{}.comment(v).comment() == v);
}
