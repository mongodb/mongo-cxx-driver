...
TEST_CASE("ownership", "[mongocxx][v1][find_one_and_delete_options]") {
    find_one_and_delete_options source;
    find_one_and_delete_options target;
...
TEST_CASE("default", "[mongocxx][v1][find_one_and_delete_options]") {
    find_one_and_delete_options const opts;
...
TEST_CASE("collation", "[mongocxx][v1][find_one_and_delete_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(find_one_and_delete_options{}.collation(v.value()).collation() == v.view());
}

TEST_CASE("max_time", "[mongocxx][v1][find_one_and_delete_options]") {
    ...
    CHECK(find_one_and_delete_options{}.max_time(v).max_time() == v);
}

TEST_CASE("projection", "[mongocxx][v1][find_one_and_delete_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(find_one_and_delete_options{}.projection(v.value()).projection() == v.view());
}

TEST_CASE("sort", "[mongocxx][v1][find_one_and_delete_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));
...
TEST_CASE("write_concern", "[mongocxx][v1][find_one_and_delete_options]") {
    ...
    CHECK(find_one_and_delete_options{}.write_concern(v).write_concern() == v);
}

TEST_CASE("hint", "[mongocxx][v1][find_one_and_delete_options]") {
    auto const v = GENERATE(values({
        v1::hint{"abc"},
        v1::hint{scoped_bson{R"({"x": 1})"}.value()},
    }));

    CHECK(find_one_and_delete_options{}.hint(v).hint() == v);
}

TEST_CASE("let", "[mongocxx][v1][find_one_and_delete_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(find_one_and_delete_options{}.let(v.value()).let() == v.view());
}

TEST_CASE("comment", "[mongocxx][v1][find_one_and_delete_options]") {
    ...
    CHECK(find_one_and_delete_options{}.comment(v).comment() == v);
}
