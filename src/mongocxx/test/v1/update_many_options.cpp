...
TEST_CASE("default", "[mongocxx][v1][update_many_options]") {
...
TEST_CASE("bypass_document_validation", "[mongocxx][v1][update_many_options]") {
    auto const v = GENERATE(false, true);

    CHECK(update_many_options{}.bypass_document_validation(v).bypass_document_validation() == v);
}

TEST_CASE("collation", "[mongocxx][v1][update_many_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(update_many_options{}.collation(v.value()).collation() == v.view());
}

TEST_CASE("hint", "[mongocxx][v1][update_many_options]") {
    auto const v = GENERATE(values({
        v1::hint{"abc"},
        v1::hint{scoped_bson{R"({"x": 1})"}.value()},
    }));

    CHECK(update_many_options{}.hint(v).hint() == v);
}

TEST_CASE("let", "[mongocxx][v1][update_many_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));
...
TEST_CASE("comment", "[mongocxx][v1][update_many_options]") {
    ...
    CHECK(update_many_options{}.comment(v).comment() == v);
}

TEST_CASE("upsert", "[mongocxx][v1][update_many_options]") {
    auto const v = GENERATE(false, true);

    CHECK(update_many_options{}.upsert(v).upsert() == v);
}

TEST_CASE("read_concern", "[mongocxx][v1][update_many_options]") {
    ...
    CHECK(update_many_options{}.read_concern(v).read_concern() == v);
}

TEST_CASE("write_concern", "[mongocxx][v1][update_many_options]") {
    ...
    CHECK(update_many_options{}.write_concern(v).write_concern() == v);
}

TEST_CASE("array_filters", "[mongocxx][v1][update_many_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
    ...
    CHECK(
        update_many_options{}.array_filters(bsoncxx::v1::array::value{v.array_view()}).array_filters() ==
        v.array_view());
}
