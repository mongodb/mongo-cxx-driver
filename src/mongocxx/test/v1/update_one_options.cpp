...
TEST_CASE("ownership", "[mongocxx][v1][update_one_options]") {
    update_one_options source;
    update_one_options target;
...
TEST_CASE("default", "[mongocxx][v1][update_one_options]") {
...
TEST_CASE("bypass_document_validation", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(false, true);

    CHECK(update_one_options{}.bypass_document_validation(v).bypass_document_validation() == v);
}

TEST_CASE("collation", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(update_one_options{}.collation(v.value()).collation() == v.view());
}

TEST_CASE("hint", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(values({
        v1::hint{"abc"},
        v1::hint{scoped_bson{R"({"x": 1})"}.value()},
    }));

    CHECK(update_one_options{}.hint(v).hint() == v);
}

TEST_CASE("let", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(update_one_options{}.let(v.value()).let() == v.view());
}

TEST_CASE("sort", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
        scoped_bson{R"({"x": 1})"},
    }));

    CHECK(update_one_options{}.sort(v.value()).sort() == v.view());
}

TEST_CASE("comment", "[mongocxx][v1][update_one_options]") {
    ...
    CHECK(update_one_options{}.comment(v).comment() == v);
}

TEST_CASE("upsert", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(false, true);
...
TEST_CASE("write_concern", "[mongocxx][v1][update_one_options]") {
    ...
    CHECK(update_one_options{}.write_concern(v).write_concern() == v);
}

TEST_CASE("array_filters", "[mongocxx][v1][update_one_options]") {
    auto const v = GENERATE(values({
        scoped_bson{},
    ...
    CHECK(
        update_one_options{}.array_filters(bsoncxx::v1::array::value{v.array_view()}).array_filters() ==
        v.array_view());
}
