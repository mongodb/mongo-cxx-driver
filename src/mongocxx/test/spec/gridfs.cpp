...
TEST_CASE("GridFS spec automated tests", "[gridfs_spec]") {
    ...
    auto tests_path = std::getenv("GRIDFS_TESTS_PATH")
                          ? std::getenv("GRIDFS_TESTS_PATH")
                          : std::string{MONGOCXX_SOURCE_DIR} + "/data/gridfs";
    mongocxx::spec::run_tests_in_suite(tests_path, cb);
}
}  // namespace
