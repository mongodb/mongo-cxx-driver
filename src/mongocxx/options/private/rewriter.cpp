...
find rewriter::rewrite_find_modifiers(const find& options) {
    ...
    // Ideally, we'd like to return an options::find object with the 'modifiers' option not engaged,
    // but the current options::find API does not provide a mechanism to clear an existing option.
    // While not ideal, it's sufficient for our purposes to set 'modifiers' to an empty view here
    // (the alternative would be to build up a new options object from scratch, but this would be
    // verbose and error-prone).
    converted_options.modifiers(bsoncxx::document::view{});
