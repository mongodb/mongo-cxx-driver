...
static unique_server_api make_server_api(const server_api& opts) {
    ...
    if (!result) {
        throw mongocxx::invalid_parameter("invalid server API version" + server_api::version_to_string(opts.get_version()));
    }

    auto mongoc_server_api_opts = libmongoc::server_api_new(mongoc_api_version);
    if (!mongoc_server_api_opts) {
        throw create_resource_fail { "could not create server API" };
    }
