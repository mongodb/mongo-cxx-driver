...
namespace {
    ...
    TEST_F(DBClientTest, CopyDatabaseMONGODBCR) {
        ...
        if (serverGTE(&c, 2, 4) && !serverGTE(&c, 2, 7)) {
            std::string errmsg;
            ...
            ASSERT_TRUE(c.copyDatabase(TEST_DB, "copy", _uri,
                                       "MONGODB-CR", "jane", "pwd"));
    ...
    TEST_F(DBClientTest, CopyDatabaseSCRAMSHA1) {
        ...
        if (serverGTE(&c, 2, 7)) {
            if (kCompiledWithSSL) {
                ...
                ASSERT_THROWS(c.copyDatabase(TEST_DB, "copy", _uri, "SCRAM-SHA-1",
                                             "user", "pass"), UserException);
            }
        }
    }

    TEST_F(DBClientTest, CopyDatabaseInvalidMechanism) {
        ASSERT_THROWS(c.copyDatabase(TEST_DB, "copy", _uri, "BLOOP",
                                     "user", "pass"), UserException);
    }
