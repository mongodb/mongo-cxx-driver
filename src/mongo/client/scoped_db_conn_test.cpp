...
namespace mongo {
    ...
    class TCPSocket {
        public:
            ...
            TCPSocket* accept() {
                ...
                int client_fd = ::accept(_fd, client_sa.get_address(), &client_sa.size);
                if (client_fd > 0) {
    ...
    class TCPServer {
        public:
            ...
            void start() {
                while (TCPSocket* client = _server_sock.accept()) {
                    if (client) {
                        _client_socks.push_back(client);
                        //std::cout << "[SERVER] " << _client_socks.size()
                            //<< " clients" << std::endl;
                    }
                }
            }
