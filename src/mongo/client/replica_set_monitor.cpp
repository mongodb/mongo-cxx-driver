...
public:
    ReplicaSetMonitorWatcher() : _monitorMutex(), _started(false), _stopRequested(false) {}
...
boost::shared_ptr<DBClientConnection> ConnectionCache::getConnectionTo(const HostAndPort& host) {
    {
    ...
    if (!conn) {
        throw DBException(
            str::stream() << "failed to connect to " << host.toString() << ": " << errmsg, 0);
    }
    return conn;
}
