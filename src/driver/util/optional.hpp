...
class __optional_storage {
   protected:
    ...
    __optional_storage(value_type&& __v)
        : __val_(std::move(__v)), __engaged_(true) {}
...
class optional : private __optional_storage<_Tp> {
   ...
   public:
    ...
    optional& operator=(optional&& __opt) {
        if (this->__engaged_ == __opt.__engaged_) {
            ...
            if (this->__engaged_)
                ...
                ::new (std::addressof(this->__val_))
                    value_type(std::move(__opt.__val_));
            this->__engaged_ = __opt.__engaged_;
        }
        return *this;
    }
    ...
    template <class _Up,
              class = typename std::enable_if<
                  std::is_same<typename std::remove_reference<_Up>::type,
                               value_type>::value&&
    ...
    optional& operator=(_Up&& __v) {
        if (this->__engaged_)
            ...
            ::new (std::addressof(this->__val_))
                value_type(std::forward<_Up>(__v));
    ...
    void emplace(std::initializer_list<_Up> __il, _Args&&... __args) {
        *this = nullopt;
        ::new (std::addressof(this->__val_))
            value_type(__il, std::forward<_Args>(__args)...);
        this->__engaged_ = true;
    }
    ...
    void swap(optional& __opt) {
        ...
        if (this->__engaged_ == __opt.__engaged_) {
            ...
            if (this->__engaged_) {
                ::new (std::addressof(__opt.__val_))
                    value_type(std::move(this->__val_));
                this->__val_.~value_type();
            } else {
                ::new (std::addressof(this->__val_))
                    value_type(std::move(__opt.__val_));
   ...
   private:
    ...
    value_type const* __operator_arrow(std::false_type) const {
        return &this->__val_;
    }
};
